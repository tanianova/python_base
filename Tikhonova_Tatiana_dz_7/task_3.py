...
class Cell:
    ...
    def __sub__(self, other):
        try:
            if self.cell_quantity - other.cell_quantity >= 0:
                ...
                return f'разность количества ячеек двух клеток меньше 0'
        except Exception as e:
            return f'error {e}'

    def __mul__(self, other):
        if isinstance(self.cell_quantity and other.cell_quantity, int):
            return Cell(self.cell_quantity * other.cell_quantity)
        else:
            return f'type error'
    ...
    def make_order(self, cell_quantity_in_line):
        lines = int(self.cell_quantity / cell_quantity_in_line)   #колво строк
        new_row = cell_quantity_in_line * '*' + '\n'    #преобразованная строка из * с переносом
        if self.cell_quantity <= cell_quantity_in_line:
            return self.cell_quantity * '*'
        else:
            return new_row * lines + self.cell_quantity % cell_quantity_in_line * '*'
